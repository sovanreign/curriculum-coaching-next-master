...
export async function GET(req: NextRequest) {
  try {
    ...
    const baseUrl = process.env.STUDENTS_API_URL_DEV!;
    ...
    return NextResponse.json(
      {
        success: false,
        message: result.message || "Failed to fetch student data",
      },
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Construct the request URL
    const baseUrl = process.env.STUDENTS_CREATE_API_URL_DEV!;
    const url = new URL(baseUrl);
